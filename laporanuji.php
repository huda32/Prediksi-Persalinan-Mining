...
class laporanuji
{
    public function getDatUser()
    {
        ...
        $pdf->Cell(270,10,'UJI RULE POHON KEPUTUSAN',0,1,'C');
        ...
        $rsl  = mysqli_query($db_object->getMySQL(), "SELECT *  FROM data_uji");
 
        foreach ($rsl as $row) {
            $pdf->Ln();
            
                $pdf->Cell(8, 10, $row['id'], 1);
                $pdf->Cell(28, 10, $row['nama'], 1);
